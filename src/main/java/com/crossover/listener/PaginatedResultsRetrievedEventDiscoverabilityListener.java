...
@Component
public class PaginatedResultsRetrievedEventDiscoverabilityListener implements
  ...
  public void onApplicationEvent(final PaginatedResultsRetrievedEvent ev) {
    ...
    addLinkHeaderOnPagedResourceRetrieval(ev.getUriBuilder(), ev.getResponse(),
        ev.getPage(), ev.getTotalPages(), ev.getPageSize());
  }
  ...
  String constructLastPageUri(final UriComponentsBuilder uriBuilder, final int totalPages,
      final int size) {
